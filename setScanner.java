...
public class setScanner {
    ...
    private char[] charArray; // to hold the scanned line of input
    private Set<Character> letterCharSet = new TreeSet<Character>();
    private Set<Character> nonLetterCharSet = new TreeSet<Character>();
    private String[] reserveWords = {

    "program", "identifier", "var", "begin", "end", "if", "else", "endif",
            "nat", "set", "natconstant", "{", "}", "(", ")", ";", ".", ",",
            ":=", "<=", "=", "not", "*", "+", "\\", "-", "in", "then", "CMP",
            "eof", "unrecognized" };
    ...
    public setScanner(Scanner s) throws Exception {
        if (s == null)
            throw new Exception(
                    "Invalid Scanner input to setScanner constructor.");

        src = s;
        // scan a line into an array of characters
        if (src.hasNextLine()) {
            currLineNumber++;
            charArray = src.nextLine().toCharArray();
        }
        currPos = 0;

        // add the starting chars of each token to a set
        letterCharSet.add('p');
        letterCharSet.add('i');
        letterCharSet.add('v');
        letterCharSet.add('b');
    ...
    public void consume() {
      ...
      */
        ...
        if (currPos == charArray.length - 1) {
            if (nonLetterCharSet.contains(charArray[currPos])){
                tokenString = tokenString + charArray[currPos++];
                for (int j = 0; j < reserveWords.length; j++) {
                    if (tokenString.equals(reserveWords[j])) {
                        tokenType = j;
                        break;
                    }
                }
        ...
        while (src.hasNextLine() && (charArray.length == 0 ||currPos >= charArray.length - 1 || Character.isWhitespace(charArray[currPos]))) {

            // advance to the next character that is not white space
            while (currPos < charArray.length - 1
                    && Character.isWhitespace(charArray[currPos])) {
                currPos++;
            }
            if (currPos >= charArray.length - 1) {
                // for non letter special characters 
                if (currPos == charArray.length - 1 && (nonLetterCharSet.contains(charArray[currPos]) || (!Character.isDigit(charArray[currPos]) && !Character.isLetter(charArray[currPos])) && !Character.isWhitespace(charArray[currPos]))) {
                    break;
                }
                if (charArray.length == 1 && currPos == charArray.length - 1 && (Character.isLetter(charArray[currPos]) || Character.isDigit(charArray[currPos]))) {
                    break;
                }
                if (currPos == charArray.length - 1 && Character.isLetter(charArray[currPos])) {
                    break;
                }
                currLineNumber++;
                charArray = src.nextLine().toCharArray();
        ...
        if (!src.hasNextLine() || charArray == null) {
            ...
            // the current character is not in the set of starting letters for
            // tokens
            // scan to the next white space char and construct the unrecognized
            // character
            if (!letterCharSet.contains(charArray[currPos])&& !nonLetterCharSet.contains(charArray[currPos])&& !Character.isLetter(charArray[currPos]) && !Character.isDigit(charArray[currPos])) {
                int i = currPos;

                while (i <= charArray.length - 1
                        && !Character.isWhitespace(charArray[i])) {
                    tokenString = tokenString + charArray[i++];
                }
                currToken = new Token(30, currLineNumber, tokenString);
                currPos = i;
                tokenString = "";
            } else {
                if (Character.isDigit(charArray[currPos])) {
                    if (charArray[currPos] != '0') {
                        int i = currPos;
                        while (i < charArray.length - 1 && !Character.isWhitespace(charArray[i]) && !nonLetterCharSet.contains(charArray[i]) && !Character.isLetter(charArray[i])) {
                            tokenString = tokenString + charArray[i++];
                        }
                        if (i == charArray.length - 1 && !Character.isWhitespace(charArray[i]) && !nonLetterCharSet.contains(charArray[i]) && !Character.isLetter(charArray[i])) {
                            tokenString = tokenString + charArray[i++];
                        }
                        currPos = i;
                    ...
                    currToken = new Token(Token.NATCONST, currLineNumber,
                            tokenString);
                    tokenString = "";

                    // for letter characters
                } else if (letterCharSet.contains(charArray[currPos]) || Character.isLetter(charArray[currPos])) {
                    int i = currPos;
                    while (i < charArray.length-1 && !Character.isWhitespace(charArray[i]) && !nonLetterCharSet.contains(charArray[i])) {
                        tokenString = tokenString + charArray[i++];
                    }
                    if (i == charArray.length - 1 && !Character.isWhitespace(charArray[i]) && !nonLetterCharSet.contains(charArray[i])) {
                        tokenString = tokenString + charArray[i++];
                    }
                    for (int j = 0; j < reserveWords.length; j++) {
                        if (tokenString.equals(reserveWords[j])) {
                ...
                } else if (!Character.isLetter(charArray[currPos])) {
                    int i = currPos;
                    tokenString = tokenString + charArray[i++];
                    if (tokenString.equals("<") || tokenString.equals(":")) {
                        if (i < charArray.length - 1 && charArray[i] == '=') {
                            ...
                            while (i < charArray.length-1 && !Character.isWhitespace(charArray[i])) {
                                tokenString = tokenString + charArray[i++];
                            }
                        }
                    }                     
